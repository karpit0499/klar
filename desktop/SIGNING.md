...
9. Find the generated `.app` and verify every required property before any
   ...
   shasum -a 256 "release/desktop/Klar-2.6.1-mac-arm64.dmg" "release/desktop/Klar-2.6.1-mac-arm64.zip"
   ```

   The x64 unpacked app is `release/desktop/mac/Klar.app`; run the same four
   checks against it. Its public artifacts are
   `release/desktop/Klar-2.6.1-mac-x64.dmg` and
   `release/desktop/Klar-2.6.1-mac-x64.zip`.
...
```powershell
$artifact = ".\release\desktop\Klar-Setup-2.6.1-win-x64.exe"
