...
export function ResumeReupload({ apiKey, requireGroq, onReplace }: {
  ...
  const { locale, t } = useLocale(); const de = locale === 'de'
  ...
  async function confirm() {
    ...
    try { await onReplace(preview); setPreview(null); setPasted('') }
      ...
      setError(toAppError(caught, {
        category: 'storage',
        message: t('resume.replaceFailed'),
        dataSafe: true,
        available: t('resume.currentAvailable'),
  ...
  if (preview) return (
    <div className="mt-4">
      <div className="mb-3 rounded-md border border-accent bg-accent-tint p-3 text-sm text-accent">
        {de
          ? 'Vorschau: Prüfe alle Abschnitte. Beim Bestätigen legt Klar automatisch eine wiederherstellbare Version des aktuellen Profils an und ersetzt es vollständig.'
          : 'Preview: review every section. On confirmation, Klar automatically saves a restorable version of the current profile and replaces it in full.'}
      </div>
      <ResumeEditor
        value={preview}
        onChange={setPreview}
        onSave={() => void confirm()}
        busy={busy === 'saving'}
        saveLabel={de ? 'Vorschau bestätigen und ersetzen' : 'Confirm preview and replace'}
      />
      {error && <div className="mt-3"><ErrorNotice error={error} /></div>}
      <div className="mt-3">
        <Button variant="ghost" onClick={() => { setPreview(null); setError(null) }}>
          {de ? 'Abbrechen' : 'Cancel'}
        </Button>
      </div>
    </div>
