...
export function DashboardStep({
  ...
  async function handlePhoto(file: File) {
    ...
    try {
      ...
      setPhotoErr(e instanceof Error ? e.message : 'Could not read that image.')
    }
  }
      ...
      <Card className="p-6">
            ...
            {editing ? (
              <div className="space-y-3">
                <Field label="Display name">
                  <TextInput value={form.displayName} onChange={(e) => set({ displayName: e.target.value })} placeholder="Your name" />
                </Field>
              ...
              <>
                <h2 className="text-xl font-semibold text-gray-900">{form.displayName || 'Your dashboard'}</h2>
                {form.headline && <p className="text-sm text-gray-600">{form.headline}</p>}
                {form.location && <p className="text-sm text-gray-500">{form.location}</p>}
              </>
            )}
          </div>
          <div className="shrink-0">
            {editing ? (
              <Button onClick={save} className="px-3 py-1.5">Save</Button>
            ) : (
              <Button variant="ghost" onClick={() => setEditing(true)} className="px-3 py-1.5">Edit</Button>
            )}
          </div>
        </div>
        ...
        {editing && (
          <div className="mt-4 space-y-4">
            <div>
              <p className="mb-1 text-sm font-medium text-gray-700">Photo</p>
              <input
                ref={fileRef}
                type="file"
                accept="image/*"
              ...
              <div className="flex items-center gap-2">
                <Button variant="ghost" className="px-3 py-1.5" onClick={() => fileRef.current?.click()}>
                  Choose image
                </Button>
                {form.photoDataUrl && (
                  <Button variant="ghost" className="px-3 py-1.5" onClick={() => set({ photoDataUrl: undefined })}>
                    Remove photo
                  </Button>
                )}
              </div>
              <p className="mt-1 text-xs text-gray-500">Stored only in this browser (max 2MB).</p>
              {photoErr && <p className="mt-1 text-sm text-red-600">{photoErr}</p>}
            </div>

            <Field label="About" hint="A short description that makes this yours.">
              <textarea
                ...
                placeholder="What you're looking for, what you bring…"
              />
            </Field>

            <div>
              <p className="mb-1 text-sm font-medium text-gray-700">Links</p>
        ...
        {!editing && form.about && (
          <p className="mt-4 whitespace-pre-wrap text-sm leading-relaxed text-gray-700">{form.about}</p>
        )}
        {!editing && form.links.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-3 text-sm">
            {form.links.filter((l) => l.url).map((l, i) => (
              <a key={i} href={l.url} target="_blank" rel="noreferrer" className="text-indigo-600 underline">
                {l.label || l.url}
              </a>
            ))}
          </div>
      ...
      {!editing && (profile || prefs) && (
        <Card className="mt-4 p-6">
          ...
          <div className="mt-2 grid grid-cols-2 gap-3 text-sm text-gray-600">
            {profile && <div><span className="font-medium">Skills:</span> {profile.skills.length}</div>}
            {profile?.totalYears != null && <div><span className="font-medium">Experience:</span> {profile.totalYears}y</div>}
            {prefs && <div><span className="font-medium">Target:</span> {prefs.targetTitles.join(', ') || '—'}</div>}
            {prefs?.locations[0] && <div><span className="font-medium">Location:</span> {prefs.locations[0].city}</div>}
          </div>
        </Card>
      )}
    </div>
