  ...
  return (
    <div className="space-y-4">
      ...
      <Section title={de ? 'Zertifikate' : 'Certifications'}>
        {value.certifications.map((cert, index) => <Row key={cert.id} onUp={() => change((d) => { d.certifications = move(d.certifications, index, -1) })} onDown={() => change((d) => { d.certifications = move(d.certifications, index, 1) })} onDelete={() => remove(cert.name || 'certification', (d) => { d.certifications.splice(index, 1) })}><TextInput aria-label={de ? 'Zertifikat' : 'Certification'} value={cert.name} onChange={(e) => change((d) => { d.certifications[index].name = e.target.value })} placeholder={de ? 'Zertifikat' : 'Certification'} /><TextInput aria-label={de ? 'Aussteller' : 'Issuer'} value={cert.issuer ?? ''} onChange={(e) => change((d) => { d.certifications[index].issuer = e.target.value || undefined })} placeholder={de ? 'Aussteller' : 'Issuer'} /></Row>)}
      ...
      <Section title={de ? 'Sprachen' : 'Languages'}>
        {value.languages.map((language, index) => <Row key={language.id} onUp={() => change((d) => { d.languages = move(d.languages, index, -1) })} onDown={() => change((d) => { d.languages = move(d.languages, index, 1) })} onDelete={() => remove(language.lang || 'language', (d) => { d.languages.splice(index, 1) })}><TextInput aria-label={de ? 'Sprache' : 'Language'} value={language.lang} onChange={(e) => change((d) => { d.languages[index].lang = e.target.value })} placeholder={de ? 'Sprache' : 'Language'} /><TextInput aria-label={de ? 'Niveau' : 'Level'} value={language.level ?? ''} onChange={(e) => change((d) => { d.languages[index].level = e.target.value || undefined })} placeholder="B2" /></Row>)}
